...
function searchPanel(mode) {
  ...
  if (mode === 'model') return `<form class="search-large finder-search" data-model-form><label class="finder-input"><select name="model" aria-label="Модель техники"><option value="JCB 3CX">JCB 3CX</option><option value="JCB 4CX">JCB 4CX</option><option value="JCB 5CX">JCB 5CX</option><option value="Телескопический погрузчик JCB">Телескопический погрузчик JCB</option><option value="Другая модель JCB">Другая модель JCB</option></select></label>${submit}</form>`;
